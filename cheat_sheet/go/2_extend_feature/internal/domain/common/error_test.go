...
func TestDomainError_Option(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			TestError: NewError(ErrorCodeRemoteClientError, nil, WithStatus(status)),
			WitStatus: true,
		},
		{
			Name:       "with detail",
...
func TestDomainError_ErrorMapping(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			Name:             "message content invalid",
			TestError:        NewError(ErrorCodeMessageContentInvalid, nil),
			ExpectErrorName:  ErrorCodeMessageContentInvalid.Name,
			ExpectHTTPStatus: http.StatusBadRequest,
		}, {
			Name:             "unsupported channel type",
			TestError:        NewError(ErrorCodeUnsupportedChannelType, nil),
			ExpectErrorName:  ErrorCodeUnsupportedChannelType.Name,
			ExpectHTTPStatus: http.StatusBadRequest,
		},
		{
		...
		{
			Name:                   "remote client error",
			TestError:              NewError(ErrorCodeRemoteClientError, nil, WithStatus(http.StatusUnauthorized)),
			ExpectErrorName:        ErrorCodeRemoteClientError.Name,
			ExpectHTTPStatus:       http.StatusBadRequest,
			ExpectRemoteHTTPStatus: http.StatusUnauthorized,
		}, {
			Name:                   "remote server error",
			TestError:              NewError(ErrorCodeRemoteServerError, nil, WithStatus(http.StatusInternalServerError)),
			ExpectErrorName:        ErrorCodeRemoteServerError.Name,
			ExpectHTTPStatus:       http.StatusBadGateway,
			ExpectRemoteHTTPStatus: http.StatusInternalServerError,
		},
