...
def validate_trigger(
    ...
    Priority order according to PRD (IG Story extension):
    0. IG Story keyword triggers (highest priority)
    0.5. IG Story general triggers
    1. General keyword triggers
    ...
    # Phase 0: Check IG Story keyword triggers (highest priority)
    ig_story_keyword_triggers = [t for t in active_triggers if is_ig_story_keyword_trigger(t)]
    for trigger in ig_story_keyword_triggers:
        if matches_ig_story_keyword_trigger(message_event, trigger):
            # Check schedule constraints for IG Story keyword triggers (DATE_RANGE only)
            if trigger.trigger_schedule_type == WebhookTriggerScheduleType.DATE_RANGE and not matches_time_schedule(
                ...
                continue
            return TriggerValidationResult(trigger, "ig_story_keyword")

    # Phase 0.5: Check IG Story general triggers
    ig_story_general_triggers = [t for t in active_triggers if is_ig_story_general_trigger(t)]
    for trigger in ig_story_general_triggers:
        if matches_ig_story_general_trigger(
            message_event, trigger, business_hour_checker, organization_id, bot_timezone, organization_timezone
        ):
            return TriggerValidationResult(trigger, "ig_story_general")

    # Phase 1: Check general keyword triggers
    keyword_triggers = [t for t in active_triggers if is_keyword_trigger(t) and not t.is_ig_story_trigger()]
    ...
    if isinstance(message_event, FollowEvent):
        welcome_triggers = [t for t in active_triggers if is_welcome_trigger(t) and not t.is_ig_story_trigger()]
    ...
    general_triggers = [t for t in active_triggers if is_general_trigger(t) and not t.is_ig_story_trigger()]
...
def get_ig_story_id_from_event(event: WebhookEvent) -> str | None:
    """Extract IG Story ID from webhook event extra field.

    Args:
        event: The webhook event to extract IG story ID from

    Returns:
        IG Story ID if present in event extra field, None otherwise
    """
    extra_data = getattr(event, "extra", None)
    if not extra_data:
        return None

    story_id = extra_data.get("ig_story_id")
    return str(story_id) if story_id is not None else None


def is_ig_story_keyword_trigger(trigger_setting: WebhookTriggerSetting) -> bool:
    """Check if a trigger setting is an IG Story keyword trigger.

    Args:
        trigger_setting: The trigger setting to check

    Returns:
        True if this is an IG Story keyword trigger
    """
    return (
        trigger_setting.is_ig_story_trigger()
        and len(trigger_setting.get_ig_story_keywords()) > 0
        and trigger_setting.event_type == WebhookTriggerEventType.MESSAGE
    )


def is_ig_story_general_trigger(trigger_setting: WebhookTriggerSetting) -> bool:
    """Check if a trigger setting is an IG Story general (time-based) trigger.

    Args:
        trigger_setting: The trigger setting to check

    Returns:
        True if this is an IG Story general trigger
    """
    return (
        trigger_setting.is_ig_story_trigger()
        and len(trigger_setting.get_ig_story_keywords()) == 0  # No keywords = general
        and trigger_setting.event_type == WebhookTriggerEventType.TIME
    )


def matches_ig_story_keyword_trigger(event: WebhookEvent, trigger_setting: WebhookTriggerSetting) -> bool:
    """Check if an event matches an IG Story keyword trigger.

    Args:
        event: The webhook event to check
        trigger_setting: The IG Story keyword trigger setting

    Returns:
        True if event matches IG Story keyword trigger
    """
    # Must be a MESSAGE event
    if not isinstance(event, MessageEvent):
        return False

    # Must have IG story context
    ig_story_id = get_ig_story_id_from_event(event)
    if not ig_story_id:
        return False

    # Must match one of the configured story IDs
    if not trigger_setting.matches_ig_story(ig_story_id):
        return False

    # Must match one of the keywords
    keywords = trigger_setting.get_ig_story_keywords()
    if not keywords:
        return False
...
def matches_ig_story_general_trigger(
    event: WebhookEvent,
    trigger_setting: WebhookTriggerSetting,
    business_hour_checker: BusinessHourChecker | None = None,
    organization_id: int | None = None,
    ...
    """Check if an event matches an IG Story general (time-based) trigger.

    Args:
        event: The webhook event to check
        trigger_setting: The IG Story general trigger setting
        business_hour_checker: Business hour checker implementation
        organization_id: Organization ID for business hour checking
        bot_timezone: Bot's configured timezone for time-based comparisons
        organization_timezone: Organization's timezone for business hour checking
    ...
    Returns:
        True if event matches IG Story general trigger
    """
    # Must be a MESSAGE event
    if not isinstance(event, MessageEvent):
        return False
    ...
    # Check schedule constraints (same as general triggers)
    return matches_time_schedule(
        event, trigger_setting, business_hour_checker, organization_id, bot_timezone, organization_timezone
    )
