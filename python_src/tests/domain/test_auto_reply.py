...
from internal.domain.auto_reply.auto_reply import ChannelType, MessageEvent, validate_trigger
...
def test_no_disturb(monkeypatch):
    ...
    now = datetime(2024, 6, 1, 10, 0)
    trigger = make_trigger(trigger_code="hello", extra={"no_disturb_interval": 10}, id=99)
...
def test_general_monthly_schedule_match():
    ...
    schedule: dict[str, Any] = {"monthly": [{"day": 15, "start_time": "14:00", "end_time": "16:00"}]}
...
def test_priority_general_when_no_keyword():
    ...
    event = make_event("notmatch", ts=now)
    result = validate_trigger(event, [kw_trigger, gen_trigger], bot_timezone="Asia/Taipei")
    assert result is not None
...
def test_message_content_exact():
    ...
    event = make_event("hello", ts=datetime.now())
    result = validate_trigger(event, [trigger], bot_timezone="Asia/Taipei")
    assert result is not None
    assert result.name == "kw"
...
def test_message_content_no_keyword():
    ...
    event = make_event("notmatch", ts=datetime.now())
    result = validate_trigger(event, [trigger], bot_timezone="Asia/Taipei")
    assert result is None
...
def test_message_content_general_any():
    ...
    event = make_event("anything", ts=now)
    result = validate_trigger(event, [trigger], bot_timezone="Asia/Taipei")
    assert result is not None
    assert result.name == "gen"
...
def test_ig_story_keyword_only_triggers_on_story():
    ...
    trigger = make_trigger(
        event_type=WebhookTriggerEventType.MESSAGE, trigger_code="hello", extra={"ig_story_ids": ["story123"]}
    )
...
def test_ig_story_priority_over_general():
    ...
    gen_trigger = make_trigger(event_type=WebhookTriggerEventType.MESSAGE, trigger_code="hello", name="general_kw")
    event = make_ig_event("hello", ig_story_id="story123")
...
def test_ig_story_multiple_keywords():
    ...
    trigger = make_trigger(
        event_type=WebhookTriggerEventType.MESSAGE, trigger_code="hello,hi", extra={"ig_story_ids": ["story123"]}
    )
...
def test_ig_story_priority_system():
    ...
    gen_kw = make_trigger(event_type=WebhookTriggerEventType.MESSAGE, trigger_code="hello", name="gen_kw")
